...
namespace Assessment2
{
    ...
    public static void RemoveNum(List<int> list, int num)
    {
        if (list.Contains(num))
        {
            list.Remove(num);
            Console.WriteLine($"Removed {num} from the list.");
        }
        else
        {
            Console.WriteLine($"{num} is not present in the list.");
        }
    }
}
    ...
    int numToRemove = 3;
    ListHelper.RemoveNum(numbers, numToRemove);
...
public static int TryMe(int numerator, int denominator)
{
    ...
    int result = MathOperations.TryMe(num1, num2);
